...
模板分五个系列，**foot 文案各系列完全一致**，仅色值随所选配色替换；版式风格不同。
...
- 标题采用瑞慈原稿式「微信标题 + 账号/日期/地域低对比元信息 + 标题底线」；只有来源确有英文栏目/主题时才写英文，禁止为装饰硬加英文。不要用 template3 的居中英文眉题替代微信标题层。
- 叙事顺序优先遵循「标题/元信息 → Hero → 大留白与白色叠层 → 连续视觉叙事 → CTA → 浅蓝分隔 → 附录 → 尾部」，不要把连续视觉故事拆成泛用的绿卡/数据卡堆叠。
...
- `RuiciRelatedReading` 是可选附录（0–3 条链接），不强制加入；没有经确认链接时，不展示来源文章的无关链接。
- 静态互动提示仅在目标编辑器需要复刻参考视觉时使用，明确是非交互装饰；不应误认为或模拟微信原生点赞、分享、收藏功能。
- 不同机构文章不得默认混入小胰宝组织 footer、社交账号或官网；瑞慈版应使用机构/项目 footer 插槽，未提供机构资料时以来源/引用块结束。
- 图片使用 `width:100%;height:auto;display:block`，禁止复制来源中的固定像素尺寸、空白 `<br>` 堆叠；尾部装饰避免使用脆弱的 3D `rotateX`。

> 路由补充：用户指定"瑞慈""体检筛查""医院合作""高危转诊""医疗服务指南"时，使用 `template4-ruici`。未指定时仍默认 template3 v3.1。

### template5-khub 系列（`assets/template5-khub/`，病历记录风）

**病历记录（Patient Practice Record）**风格，提取自 Khub 公众号"确诊后，我让AI读完了自己的基因组"文章。衬线字体 + 粗黑边框 + 暖米色背景，模拟医学档案的排版质感，适合个人叙事、深度科普、罕见病记录等长文。

**Khub 视觉系统：**
- 暖米背景 `#faf7ef`：页面底色；卡片白 `#fffdf8`：内容卡片
- 深绿 `#0b5e45`：品牌条背景、标题、强调文字
- 粗黑 `#151515`：2px 实线边框、正文文字
- 黄色 `#f7c90d`：引用块左边框、底部色条
- 警示红 `#c94b3f`：疾病名强调、底部色条
- 蓝色 `#3d7daa`：底部色条

**字体系统：**
- 标题：`Georgia, "Songti SC", STSong, "Noto Serif CJK SC", serif`（衬线字体），H1 28px/900，H2 22-23px/900
- 正文：`"PingFang SC", "Microsoft YaHei", sans-serif`，15px，line-height 1.8，颜色 `#151515`

**可用资产：**
- `khub_template.html`：带渲染占位符的主模板；
- `khub_components.html`：SectionCard、SectionHeader、BodyParagraph、Blockquote、DataTable、HeaderDataRow、ColorBar、ImageFrame 等组件；
- `khub_foot_template.html`：病历风格 footer（白底粗黑边框 + 社区信息）；
- `khub_template_spec.md`：视觉抽取规格。

**模块规则：**
- 所有内容块使用 `KhubSectionCard`（白底 + 2px 粗黑边框 + 8px 圆角）包裹。
- 章节标题使用 `KhubSectionHeader`：编号 + 粗黑底线 + 衬线 H2 标题。
- `KhubBlockquote` 使用黄色左边框（5px `#f7c90d`），区别于标准灰色引用。
- `KhubColorBar` 四色条（绿40% + 黄22% + 红16% + 蓝22%）为固定尾部装饰。
- 品牌条头部使用 `<table>` 布局而非 `<section>` flex，确保微信兼容。
